...
*/
    /*$("#previewreportbtn").click(function (e) {
        ...
        if (!isError) {
            var url = "api/user/doctor/previewConsultation";
            $.ajax({
                url: url,
                type: 'POST',
                dataType: 'json',
                data: JSON.stringify(EncounterFormToJSON('#consultationform')),
...
/*    $("#rxlist").on('click', '.prxDF', function () {
    ...
    $(".cancel-common").click(function () {
        ...
        $('#iframe').attr('src', "");
        $("#doc-dash-for-dict").removeClass("dailog-show");
    });
    ...
    $(".addrxb").click(function () {
        ...
        if (medicationName != '' && strength != '' && q != '') {
            ...
            var route = $("#route").val();
            var t = $('#frequency').val() + " " + "(" + $("input:radio[name=shift]:checked").val() + ")";
   ...
   /* $("#enc_save_history").click(function (e) {
        e.stopPropagation();
...
/*    $("#chiefComplaint,#hpi,#subjective").keyup(function () {
 ...
 /*   $('#consultationform').submit(function (e) {
        e.preventDefault();
        $('#s_confirmMessage,#s_errorMessage').empty();
        var cc = $("#chiefComplaint").val();
        var hpi = $("#hpi").val();

        if ((cc != null && cc != '' && cc != 'undefined') || (hpi != null && hpi != '' && hpi != 'undefined')) {
            $("#saveEncPopup").addClass("dailog-show");
            $(".cancelapp").off().on('click', function () {
                var retVal = this.id;
                $("#saveEncPopup").removeClass("dailog-show");
                if (retVal == "Yes") {
                    $("#consultationform").find(".submit_btn").html('<img src=resources/images/red_loading.gif" id="load_btn" class="small_loading" onclick="return false;">');
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            $("#consultationform").find(".submit_btn").html('Save');
                        },
                        error: function (data) {
                            window.location = 'invalidSession';
                        }
...
/*function getEncounterLabels1(rosCategories,peCategories) {
...
function getlabdata1(data) {

    var labCategories = data['labCategories'];
    var hmtValues = data['hmtValues'];
    var sgyValues = data['sgyValues'];
    var cpgyValues = data['cpgyValues'];
    var cgyValues = data['cgyValues'];
    var bcyValues = data['bcyValues'];
    var mbgyValues = data['mbgyValues'];
    var hrtValues = data['hrtValues'];
